...
def getValidPlayerAction(player,currentPlayerTokens,opposingPlayerTokens,board,forwardRowInc):
    ...
    jumpsList=expandJumps(board,currentPlayerTokens,forwardRowInc,opposingPlayerTokens)
    print("Valid Jumps List",jumpsList)
    if len(jumpsList)>0:
        move=input("Enter jump for player "+player+" => ").upper()
        while move != "QUIT" and move not in jumpsList:
...
#add functionality for multiple options of jumps
def expandJumps(board,currentPlayerTokens,rowInc,opposingPlayerTokens):
    validJumps = listValidSingleJumps(board,currentPlayerTokens,rowInc,opposingPlayerTokens)
    newJump = True
    while newJump == True:
        addedJump = 0
        for i in range(len(validJumps)):
            row = ord(validJumps[i][-2]) - 65
            col = int(validJumps[i][-1])
            if board[row][col] in ['r','b']:
                for colInc in [1,-1]:
                    if row+rowInc*2>=0 and row+rowInc*2<=7 and col+colInc*2>=0 and col+colInc*2<=7 and board[row+rowInc][col+colInc] in opposingPlayerTokens and board[row+(2*rowInc)][col+(2*colInc)]=='':
                        if validJumps[i].find(":"+chr(row+2*rowInc+65)+str(col+colInc*2)) == -1:
                                    validJumps[i] += ":"+chr(row+2*rowInc+65)+str(col+colInc*2)
                                    addedJump = 1
            else:
                for rInc in [1,-1]:
                    for cInc in [-1,1]:
                        if (col+cInc)>=0 and (col+cInc)<=7 and (row+rInc)>=0 and (row+rInc)<=7 and board[row+rInc][col+cInc] in opposingPlayerTokens and (col+2*cInc)>=0 and (col+2*cInc)<=7 and row+(2*rInc)>=0 and row+(2*rInc)<=7 and board[row+(2*rInc)][col+(2*cInc)]=='' :
                            if validJumps[i].find(":"+chr(row+(2*rInc)+65)+str(col+(2*cInc))) == -1:
                                validJumps[i] += ":"+chr(row+(2*rInc)+65)+str(col+(2*cInc))
                                addedJump = 1
        if addedJump == 0:
            newJump = False
    return validJumps
