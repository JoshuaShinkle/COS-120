...
def becomeKing(player,currentPlayerTokens,opposingPlayerTokens,board,forwardRowInc):
    for row in range(8):
        for col in range(8):
            if board[row][col] == 'b' and row == 1:
                if board[row-1][col+1] == '' and col >= 0 and col <= 7:
                    move = chr(row+65)+str(col)+":"+chr(row-1+65)+str(col+1)
                    return move
                elif board[row-1][col-1] == '' and col >= 0 and col <= 7:
                    move = chr(row+65)+str(col)+":"+chr(row-1+65)+str(col-1)
                    return move

def jumpToKing(player,currentPlayerTokens,opposingPlayerTokens,board,forwardRowInc, expandedJumpsList):
...
def getValidPlayerAction(player,currentPlayerTokens,opposingPlayerTokens,board,forwardRowInc):
    ...
    if len(expandedJumpsList)>0:
        move = jumpToKing(player,currentPlayerTokens,opposingPlayerTokens,board,forwardRowInc, expandedJumpsList)
...
##            move=input("Enter jump for player "+player+" => ").upper()
        ...
        move = becomeKing(player,currentPlayerTokens,opposingPlayerTokens,board,forwardRowInc)
